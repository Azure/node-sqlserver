...
function validateParameters( parameters, funcName ) {
    ...
    for( var p in parameters ) {
        ...
        if( typeof parameters[p].value != parameters[p].type ) {
            ...
            throw new Error( ["[node-sqlserver] Invalid ", parameters[p].name, " passed to function ", funcName, ". Type should be ", parameters[p].type, "." ].join('') );
        }
    }
}
...
function getChunkyArgs(paramsOrCallback, callback) {
    ...
    if(( typeof paramsOrCallback == 'object' && paramsOrCallback instanceof Array == true ) &&
    ...
    if( typeof paramsOrCallback == 'undefined' && typeof callback == 'undefined' ) {

        return { params: [], callback: callback };
    }

    throw new Error( "[node-sqlserver] Invalid parameter(s) passed to function query or queryRaw." );
}
