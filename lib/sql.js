...
function readall(q, notify, ext, query, params, callback) {
    ...
    function rowsCompleted( results ) {
    ...
    function rowsAffected() {
        ...
        rowsCompleted( { meta: null, rowcount: rowCount } );
    }
    ...
    function onReadRow(completed, err, moreRows) {
        ...
        if (err) {
            ...
            rowsCompleted( { meta: meta, rows: rows } );
        }
    }
...
function queryRaw(connectionString, query, paramsOrCallback, callback) {
    ...
    chunky.callback = chunky.callback || function( err ) { throw new Error( err ); };
